...
testAdvanceGame = do
    ...
    return $ tests 10 8
        ...
        , testVal "check advanceGame 6"
                  advancegame6
                  $ show $ advanceGameState North False $ advanceGameState North False $ advanceGameState West False $ advanceGameState West False $ advanceGameState East False 
...
testAStarTreePath = return $ tests 16 10
  [
    testSet "AStarPath.Tree.Root0" (evaluateTest 0) [1, 4, 9, 18],
    testSet "AStarPath.Tree.Root1" (evaluateTest 1) [4, 9, 18] ,
    testSet "AStarPath.Tree.Root3" (evaluateTest 4) [9, 18],
    testSet "AStarPath.Tree.Root8" (evaluateTest 9) [18],
    testSet "AStarPath.Tree.Goal"  (evaluateTest 18) [] 
  ]
  where
...
checkAll = vmCheckIO $ basics ++ problemState ++ search ++ bonus
    where
        basics       = [ testEmptyGame, testAddHunter, testAddGateway, testAddTarget, testAddObstacle, testAddAll, testBehaviors, testIsTargetKilled, testAdvanceGame, testAttemptMove]
